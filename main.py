...
def patch_html(html, buttons, text_map, new_title):
    soup = BeautifulSoup(html, "html.parser")
    if new_title:
        if soup.title: soup.title.string = new_title
        else:
    ...
    for b in buttons:
        for a in soup.find_all("a", href=True):
            if a["href"].lower() == b["orig_hr"].lower() and a.get_text(strip=True).lower() == b["orig_txt"].lower():
                if b["delete"]: a.decompose()
                else:
                    a["href"] = b["new_hr"]
                    a.clear(); a.append(b["new_txt"])
...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not await owner_only(update): return ConversationHandler.END
...
async def receive_file(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not await owner_only(update): return ConversationHandler.END
    init_user_data(ctx); doc = update.message.document
...
async def show_main_menu(update_target, ctx):
    btns = ctx.user_data["btns"]; c_title = ctx.user_data.get("custom_title"); kb = []
...
async def done_uploading(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not await owner_only(update): return ConversationHandler.END
    init_user_data(ctx); files = ctx.user_data.get("files")
    if not files: return UPLOADING
    all_btns, seen = [], set()
    for content in files.values():
        for b in extract_buttons(content):
            if (b["orig_txt"].lower(), b["orig_hr"].lower()) not in seen:
                seen.add((b["orig_txt"].lower(), b["orig_hr"].lower())); all_btns.append(b)
...
async def menu_click(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "back":
        await show_main_menu(query, ctx); return SHOWING_MENU
...
async def button_sub_click(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "delete_btn":
        btns[idx]["delete"] = not btns[idx]["delete"]; await show_main_menu(query, ctx); return SHOWING_MENU
    if query.data == "edit_name":
        await query.edit_message_text("New Name?"); return EDITING_NAME_VAL
    if query.data == "edit_link":
        await query.edit_message_text("New Link?"); return EDITING_LINK_VAL
    return SHOWING_MENU
...
async def handle_val_edit(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if ctx.user_data.get("state_ref") == "name": btns[idx]["new_txt"] = val
    else: btns[idx]["new_hr"] = val
...
async def handle_search_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Replacement?"); return REPLACE_NEW_TEXT
...
async def final_finish(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "y":
        ...
        c_name, c_title, files = ctx.user_data.get("custom_filename"), ctx.user_data.get("custom_title"), ctx.user_data["files"]
...
def main():
    ...
    app.add_handler(ConversationHandler(
        ...
        states={
            UPLOADING: [MessageHandler(filters.Document.ALL, receive_file), CommandHandler("done", done_uploading)],
            SHOWING_MENU: [CallbackQueryHandler(menu_click, pattern="^(final|reset|global_replace|find_text|back|change_filename|change_title|b_.*)$"), 
                          CallbackQueryHandler(button_sub_click, pattern="^(delete_btn|edit_name|edit_link)$")],
            EDITING_NAME_VAL: [MessageHandler(filters.TEXT & ~filters.COMMAND, lambda u, c: (c.user_data.update({"state_ref": "name"}), handle_val_edit(u, c))[1])],
            EDITING_LINK_VAL: [MessageHandler(filters.TEXT & ~filters.COMMAND, lambda u, c: (c.user_data.update({"state_ref": "link"}), handle_val_edit(u, c))[1])],
            SEARCH_OLD_TEXT: [MessageHandler(filters.TEXT & ~filters.COMMAND, handle_search_text)],
            REPLACE_NEW_TEXT: [MessageHandler(filters.TEXT & ~filters.COMMAND, handle_replace_text)],
